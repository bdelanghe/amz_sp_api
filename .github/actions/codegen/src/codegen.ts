...
/**
 * Determines if the SDK should be regenerated by comparing the folder hash
 * of the current model directory with the stored hash.
...
export async function generateSdk(
  ...
  const newContentHash = await getDirectoryContentHash(outputDir);

  // Output stats
  core.info(`SDK generated for ${apiName} in ${duration.toFixed(2)} seconds with ${fileCount} files.`);
  core.info(`New content hash for generated SDK: ${newContentHash}`);
  
  // Update the version file with the new content hash
  await writeFile(versionFilePath, JSON.stringify({ hash: newContentHash }, null, 2));
