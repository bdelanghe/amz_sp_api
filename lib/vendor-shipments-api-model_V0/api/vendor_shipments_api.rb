...
module AmzSpApi::VendorShipmentsApiModelV0
  class VendorShipmentsApi
    ...
    def get_shipment_labels_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: VendorShipmentsApi.get_shipment_labels ...'
      end
      if @api_client.config.client_side_validation && opts[:'sort_order'] && !['ASC', 'DESC'].include?(opts[:'sort_order'])
        fail ArgumentError, 'invalid value for "sort_order", must be one of ASC, DESC'
      end
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: VendorShipmentsApi#get_shipment_labels\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
